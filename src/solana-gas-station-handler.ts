/**
 * The Solana leg: co-sign and broadcast a client-authored transaction, paying
 * the fee (NIP-90 kind:5096; toon-meta#163).
 *
 ...
 * transaction with this node's advertised fee-payer address as `feePayer`,
 * partially signs with its own authority keys, and submits it; the gas station
 ...
 * the gas station contributes exactly the fee-payer signature and the
 * lamports.
 *
 ...
 *
 *  (a) DEDICATED fee-payer wallet (GAS_STATION_SOLANA_SECRET_KEY). This
 *      process holds exactly one Solana key and it is used for exactly one
 *      thing: paying fees. It is not the connector's settlement key (that
 *      lives in a different container, mounted from a different file) and not
 *      a float for any other job. It holds working SOL only, so the blast
 *      radius is that float by construction.
 ...
 *      process settles Solana claims against — see deploy/connector.toml's
 *      `[settlement.solana]`.
 *
 ...
 *
 * PAYMENT: payment-oblivious — the connector in front terminates the channel
 * payment (RouteTermination) and this file never sees a claim.
 */
...
export type LoadGasStationDeps = (options: {
  ...
  /** Override the network's public cluster RPC (SOLANA_RPC_URL). */
  rpcUrl?: string;
}) => Promise<GasStationDeps>;

/**
 * Check that a 64-byte Ed25519 keypair is self-consistent — that its trailing
 * 32 bytes really are the public key of its leading 32.
 *
 * The env parser can only check the SHAPE (128 hex characters). 128 random hex
 * characters have that shape and are not a keypair, and `openssl rand -hex 64`
 * produces exactly that — a plausible thing for an operator to run when the
 * documentation says "128-char hex". Without this, such a value boots a node
 * that looks healthy, registers kind:5096, advertises it, and then fails every
 * single job with an opaque `Solana error #3704004`.
 *
 * So the entrypoint calls this at boot: a key that cannot sign should stop the
 * process, not disappoint the first caller.
 */
export async function assertSolanaKeypair(secretKey: Uint8Array): Promise<string> {
  let keyPair: Awaited<ReturnType<typeof createKeyPairFromBytes>>;
  try {
    keyPair = await createKeyPairFromBytes(secretKey);
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
    throw new Error(
      'GAS_STATION_SOLANA_SECRET_KEY is not a valid Ed25519 keypair. It must be ' +
        'a 64-byte keypair (secretKey ‖ publicKey) as 128 hex characters — the ' +
        'format `solana-keygen` writes, NOT 64 random bytes (`openssl rand -hex ' +
        `64\` produces the right length and the wrong thing). Underlying error: ${detail}`
    );
  }
  const { getAddressFromPublicKey } = await import('@solana/kit');
  return String(await getAddressFromPublicKey(keyPair.publicKey));
}
...
export const defaultLoadGasStationDeps: LoadGasStationDeps = async (
  ...
  // Both of these are reviewed literals in ./ario-programs.ts rather than
  // reads off @ar.io/sdk at runtime — the program ids are a whitelist input,
  // and a whitelist that widens on a dependency bump is not a whitelist. That
  // file's test asserts the table still matches the SDK.
  const rpcUrl = options.rpcUrl ?? SOLANA_RPC_URLS[options.network];
  const arioProgramIds = [...ARIO_PROGRAM_IDS[options.network]];
...
export interface GasStationConfig {
  ...
  solanaSecretKey: Uint8Array;
  /**
   * Override the network's public cluster RPC. The public endpoints are
   * heavily rate-limited; a deployment doing real volume wants its own.
   */
  rpcUrl?: string;
 ...
 * in-memory (v1 — a restart invalidates open quotes, which is safe:
...
export function createGasStationHandler(
...
): (ctx: GasStationHandlerContext) => Promise<GasStationHandlerResponse> {
  ...
  const failed = (
    ...
    console.warn(`[gas-station] ${phase} rejected: ${reason} — ${detail}`);
  ...
  async function runExecute(
    ...
    if (replay) {
      return acceptReceipt({ ...replay, replayed: true });
    }
    ...
    if (delta > quote.maxLamports) {
      ...
      console.error(
        `[gas-station] ALARM: simulated fee-payer delta ${delta} lamports exceeds cap ${quote.maxLamports} after static inspection passed (possible bypass attempt)`
      );
