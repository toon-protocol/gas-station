...
async function quoteThenTx(
  handler: (ctx: GasStationHandlerContext) => Promise<unknown>,
  instructions?: Instruction[]
) {
...
describe('assertSolanaKeypair', () => {
  it('accepts a real keypair and returns its address', async () => {
    const kp = await webcrypto.subtle.generateKey('Ed25519', true, ['sign', 'verify']);
    const priv = new Uint8Array(
      await webcrypto.subtle.exportKey('pkcs8', kp.privateKey as CryptoKey)
    ).slice(-32);
    const pub = new Uint8Array(
      await webcrypto.subtle.exportKey('raw', kp.publicKey as CryptoKey)
    );
    const full = new Uint8Array(64);
    full.set(priv, 0);
    full.set(pub, 32);

    const address = await assertSolanaKeypair(full);
    expect(address).toMatch(/^[1-9A-HJ-NP-Za-km-z]{32,44}$/);
  });

  it('rejects 64 random bytes with an actionable message', async () => {
    // The shape check in the entrypoint cannot catch this: 128 random hex
    // characters are the right LENGTH. `openssl rand -hex 64` is the plausible
    // wrong thing to run, so the error says so by name — otherwise the node
    // boots healthy and fails every job with an opaque Solana error code.
    const notAKeypair = new Uint8Array(64).fill(3);
    await expect(assertSolanaKeypair(notAKeypair)).rejects.toThrow(
