/**
 * The EVM leg: relay a client-signed ERC-2771 meta-transaction through a
 * trusted forwarder and pay the gas (NIP-90 kind:5098; toon-meta#261
 * decision 9, companion to the kind:5096 Solana leg).
 *
 ...
 * `TokenNetwork` as `to`; the gas station inspects it, simulates it, then
 * submits its OWN transaction calling the forwarder's `execute(request)` and
 * pays the gas. `TokenNetwork` resolves `_msgSender()` through the forwarder
 * back to the client's `from` address (ERC-2771), so the client stays the
 * author and authority of the call — the gas station contributes only gas.
 *
 ...
 * {@link EvmGasStationConfig.chains} (chain id, RPC, forwarder + TokenNetwork
 * addresses, a funded relayer key) — no code change. See
 * `resolveEvmGasStationEnv` in `src/entrypoint-gas-station.ts` for the
 ...
 * kept in-memory (v1 — a restart invalidates open quotes, which is
...
export function createEvmGasStationHandler(
  ...
  const failed = (
    ...
    console.warn(`[gas-station] evm ${phase} rejected: ${reason} — ${detail}`);
  ...
  async function runQuote(event: NostrEvent): Promise<GasStationHandlerResponse> {
  ...
  async function runExecute(event: NostrEvent): Promise<GasStationHandlerResponse> {
    const chainIdRaw = paramTag(event, 'chainId');
    ...
    if (estimatedGas > policy.maxGas) {
      ...
      console.error(
        `[gas-station] evm ALARM: estimated gas ${estimatedGas} exceeds cap ${policy.maxGas} after static inspection passed (possible bypass attempt)`
      );
