 ...
 * daemon to prove what THIS script does with what it answers. render.sh
 * itself is real.
 *
 ...
 * containers, reporting success forever. TOON_Network#164 ports the same fix
 * to this bundle: `deploy/.applied` (gitignored) names the last commit a run
 * actually finished applying AND verifying, written only at the very end, so
 * the NEXT run compares HEAD to that -- not to what fetch just brought back
 * -- and retries, and reports, the exact same failure on every run until it
 * is fixed.
 *
...
function freshOrigin(): { dir: string; sha: string } {
  const dir = tmp('gas-station-origin-');
...
function addFixtureRequiredVar(originDir: string): string {
  ...
  const after = before.replace(
    marker,
    `${marker}\n: "\${GAS_STATION_FIXTURE_FLAG:?set GAS_STATION_FIXTURE_FLAG in .env (deploy/.env.example lists every required variable; this one is a TOON_Network#164 test fixture)}"\n`
  );
  writeFileSync(path, after);
  return commitAll(
    originDir,
    'render.sh now needs GAS_STATION_FIXTURE_FLAG'
  );
}
...
// Every call auto-apply.sh itself makes: ps -q, pull, up -d, and inspect for
// the health wait. This bundle's auto-apply.sh never calls curl -- unlike
// store's copy, it has no [node]/GET-/ilp activation check.
...
let stubBin: string;
beforeAll(() => {
  stubBin = tmp('gas-station-stub-bin-');
...
describe('a render that fails once after a fast-forward (TOON_Network#164, porting TOON_Network#160)', () => {
  it('is retried and reported on every run -- even one whose fetch brings back nothing new -- and applies once .env is fixed', () => {
    ...
    expect(second.stderr).toContain('GAS_STATION_FIXTURE_FLAG');
    ...
    expect(third.stderr).toContain('GAS_STATION_FIXTURE_FLAG');
    ...
    // their own .env.
    writeEnv(box, { ...ENV, GAS_STATION_FIXTURE_FLAG: 'ok' });
